...
function rebuildIndexesUnlocked(root: string): { modules: number; tasks: number; memories: number; skills: number; runs: number } {
  ...
  const tasks = listFiles(qaPath(root, 'modules'), path => /\/tasks\/[^/]+\/task\.json$/.test(path)).map(path => {
    ...
    const operationRoot = join(path, '..', 'operation-plans');
    const operationCount = listFiles(operationRoot, operation => operation.endsWith('.json') && !operation.endsWith('/current.json')).length;
    return { id: item.metadata.id, moduleId: item.metadata.moduleId, name: item.metadata.name, priority: item.metadata.priority, status: item.metadata.status, executionStatus: 'never_run', path: relative(qaPath(root), path), scenarioCount: item.scenarioRefs?.length ?? 0, operationCount, operationPlanRefs: item.operationPlanRefs, regressionSuiteRef: item.regressionSuiteRef, tags: item.metadata.tags, updatedAt: item.updatedAt };
  });
  const memoryFiles = [
