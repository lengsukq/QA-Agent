...
export const builtInSkills: QaSkillManifest[] = [
  { apiVersion: 'qa-agent/v2', kind: 'Skill', metadata: { name: 'execution.contract', version: '3.0.0', description: 'Validate host-supplied UI results against the approved test plan and safety policy.', lifecycle: 'active' }, requirements: { capabilities: { required: [], optional: ['browser.interact', 'android.adb', 'ios.simulator.interact'] } }, safety: { riskLevel: 'medium', writeAccess: false }, outputs: { type: 'object', properties: { acceptedSteps: { type: 'array' } } } },
  { apiVersion: 'qa-agent/v2', kind: 'Skill', metadata: { name: 'evidence.record', version: '3.0.0', description: 'Import screenshots, logs, traces, source findings, and artifacts produced by host tools.', lifecycle: 'active' }, requirements: { capabilities: { required: [], optional: ['browser.inspect', 'network.read', 'logs.read', 'source.readonly'] } }, safety: { riskLevel: 'low', writeAccess: false }, outputs: { type: 'object', properties: { evidence: { type: 'array' } } } },
  { apiVersion: 'qa-agent/v2', kind: 'Skill', metadata: { name: 'python.regression', version: '1.0.0', description: 'Validate, publish, run, and report user-reviewed Python regression scripts derived from completed Runtime Runs.', lifecycle: 'active' }, requirements: { capabilities: { required: [], optional: ['browser.interact', 'android.adb', 'ios.simulator.interact'] } }, safety: { riskLevel: 'medium', writeAccess: false }, outputs: { type: 'object', properties: { scriptId: { type: 'string' }, runId: { type: 'string' }, reportPath: { type: 'string' } } } },
