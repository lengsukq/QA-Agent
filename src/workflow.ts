...
function todos(moduleReady: boolean, taskReady: boolean, planReady: boolean, platformDeclared: boolean, questionsResolved: boolean, requirementsConfirmed: boolean, approvalRequired: boolean, executionAuthorized: boolean, capabilityReady: boolean, run?: TestRun, finalizationRequired = false, finalizationCurrent = false): WorkflowTodo[] {
  ...
  return [
    ...
    { id: 'platform-declaration', title: 'Ask the QA to declare Web or iOS Simulator for this TestPlan', status: platformDeclared ? 'completed' : planReady ? 'blocked' : 'pending', blocking: true },
...
export function workflowStatus(root: string, moduleId: string, taskId: string, request?: string): QaWorkflowState {
  ...
  else if (!platformDeclared) { status = 'approval_required'; phase = 'planning'; reasonCode = 'platform_declaration_required'; nextActions = [{ id: 'declare_test_platform', command: 'qa-agent plan apply', description: `${PLATFORM_DECLARATION_PROMPT_ZH}。将声明写入 PlanDraft.platformDeclaration 后重新应用计划。`, requiresHuman: true, requiredActor: 'human', blockingGate: 'platform_declared' }]; }
  ...
  const gates: WorkflowGate[] = [gate('task_assets_ready', taskReady, taskReady ? 'task_assets_ready' : 'task_assets_missing', 'runtime'), gate('task_plan_ready', planReady, planReady ? 'task_prd_and_detailed_steps_ready' : 'task_prd_or_detailed_steps_missing', 'agent', task ? testPlanHash(task) : undefined, taskReady), gate('platform_declared', platformDeclared, platformDeclared ? 'qa_declared_test_platform' : 'test_platform_declaration_required', 'human', task ? testPlanHash(task) : undefined, planReady), gate('requirement_questions_resolved', questionsResolved, questionsResolved ? 'no_unresolved_qa_questions' : 'qa_questions_unresolved', 'human', task ? testPlanHash(task) : undefined, planReady && platformDeclared), gate('test_plan_requirements_confirmed', requirementsConfirmed, requirementsConfirmed ? 'qa_confirmed_prd_matches_requirements' : 'qa_prd_confirmation_required', 'human', task ? testPlanHash(task) : undefined, planReady && platformDeclared && questionsResolved), gate('test_plan_approved', executionAuthorized, executionAuthorized ? 'explicit_start_confirmation_recorded' : 'explicit_start_confirmation_required', 'human', task ? testPlanHash(task) : undefined, approvalRequired && requirementsConfirmed), gate('host_capabilities_ready', capabilityReady, capabilityReady ? 'host_capabilities_ready' : 'host_capability_missing', 'host', undefined, executionAuthorized), gate('task_assets_finalized', quickFinalizationCurrent, quickFinalizationCurrent ? 'task_assets_finalized' : 'task_finalization_pending', 'runtime', task?.finalization?.artifactHash, quickFinalizationRequired)];
