...
import { approvalIsCurrent, confirmationMode, MERGED_TEST_CONFIRMATION_ZH, PLAN_REQUIREMENTS_CONFIRMATION_ZH, planReviewIsCurrent, requiresTestPlanApproval, START_TEST_CONFIRMATION_ZH, testPlanHash } from './approval.ts';
...
import type { ConfirmationMode, NextAction, QaWorkflowState, RiskLevel, TestRun, WorkflowGate, WorkflowPhase, WorkflowTodo } from './types.ts';
...
function todos(moduleReady: boolean, taskReady: boolean, planReady: boolean, platformDeclared: boolean, questionsResolved: boolean, requirementsConfirmed: boolean, approvalRequired: boolean, executionAuthorized: boolean, capabilityReady: boolean, mode: ConfirmationMode, run?: TestRun, finalizationRequired = false, finalizationCurrent = false): WorkflowTodo[] {
  ...
  return [
    ...
    { id: 'platform-declaration', title: 'Agent determines and records the unique Web or iOS Simulator platform', status: platformDeclared ? 'completed' : planReady ? 'in_progress' : 'pending', blocking: true },
    ...
    { id: 'requirements-review', title: `Wait for the QA to reply “${mode === 'merged' ? MERGED_TEST_CONFIRMATION_ZH : PLAN_REQUIREMENTS_CONFIRMATION_ZH}”`, status: requirementsConfirmed ? 'completed' : questionsResolved ? 'blocked' : 'pending', blocking: true },
    { id: 'approval', title: mode === 'merged' ? 'Merged confirmation records plan review and start authorization' : `Wait for the QA to reply “${START_TEST_CONFIRMATION_ZH}”`, status: executionAuthorized ? 'completed' : requirementsConfirmed ? 'blocked' : 'pending', blocking: approvalRequired && mode === 'strict' },
...
export function workflowStatus(root: string, moduleId: string, taskId: string, request?: string): QaWorkflowState {
  ...
  const mode = task ? confirmationMode(task) : 'strict';
  const requirementsConfirmed = Boolean(task && planReady && platformDeclared && questionsResolved && planReviewIsCurrent(task));
  ...
  else if (!platformDeclared) { status = 'setup_required'; phase = 'planning'; reasonCode = 'agent_platform_inference_required'; nextActions = [{ id: 'infer_test_platform', command: 'qa-agent plan apply', description: `Inspect project source, configuration, entry points, installed targets, and available capabilities; write the unique Web or iOS platform to PlanDraft.platformDeclaration, then reapply the PlanDraft. ${PLATFORM_DECLARATION_PROMPT_ZH}`, requiresHuman: false, requiredActor: 'agent', blockingGate: 'platform_declared' }]; }
  ...
  else if (!requirementsConfirmed) { status = 'approval_required'; phase = 'approval'; reasonCode = mode === 'merged' ? 'merged_confirmation_required' : 'test_plan_requirements_confirmation_required'; nextActions = [{ id: mode === 'merged' ? 'request_merged_confirmation' : 'request_test_plan_requirements_confirmation', command: 'qa-agent plan review', description: `Present the complete Task PRD and wait for the exact QA reply “${mode === 'merged' ? MERGED_TEST_CONFIRMATION_ZH : PLAN_REQUIREMENTS_CONFIRMATION_ZH}”.`, requiresHuman: true, requiredActor: 'human', blockingGate: 'test_plan_requirements_confirmed' }]; }
  else if (!executionAuthorized) { status = 'approval_required'; phase = 'approval'; reasonCode = mode === 'merged' ? 'merged_confirmation_incomplete' : 'explicit_start_confirmation_required'; nextActions = [{ id: 'request_test_plan_approval', command: 'qa-agent review', description: mode === 'merged' ? `The merged confirmation is incomplete. Request “${MERGED_TEST_CONFIRMATION_ZH}” through qa-agent plan review.` : `The PRD is confirmed. Wait for the separate exact QA reply “${START_TEST_CONFIRMATION_ZH}” before execution.`, requiresHuman: true, requiredActor: 'human', blockingGate: 'test_plan_approved' }]; }
  ...
  else if (!capabilityReady) { status = 'blocked'; phase = 'preflight'; reasonCode = 'runner_prerequisite_missing'; nextActions = [{ id: 'run_runner_doctor', command: `qa-agent doctor --platforms ${task?.scope.platforms[0] ?? 'web'}`, description: `Run Doctor and fix the built-in Runner prerequisites: ${capabilityStatus?.missing.join(', ') || 'unknown'}. Do not call MCP or direct UI tools.`, requiresHuman: false, requiredActor: 'host', blockingGate: 'host_capabilities_ready' }]; }
  ...
  const gates: WorkflowGate[] = [gate('task_assets_ready', taskReady, taskReady ? 'task_assets_ready' : 'task_assets_missing', 'runtime'), gate('task_plan_ready', planReady, planReady ? 'task_prd_and_detailed_steps_ready' : 'task_prd_or_detailed_steps_missing', 'agent', task ? testPlanHash(task) : undefined, taskReady), gate('platform_declared', platformDeclared, platformDeclared ? 'agent_or_user_declared_test_platform' : 'agent_platform_inference_required', 'agent', task ? testPlanHash(task) : undefined, planReady), gate('requirement_questions_resolved', questionsResolved, questionsResolved ? 'no_unresolved_qa_questions' : 'qa_questions_unresolved', 'human', task ? testPlanHash(task) : undefined, planReady && platformDeclared), gate('test_plan_requirements_confirmed', requirementsConfirmed, requirementsConfirmed ? mode === 'merged' ? 'merged_confirmation_recorded' : 'qa_confirmed_prd_matches_requirements' : mode === 'merged' ? 'merged_confirmation_required' : 'qa_prd_confirmation_required', 'human', task ? testPlanHash(task) : undefined, planReady && platformDeclared && questionsResolved), gate('test_plan_approved', executionAuthorized, executionAuthorized ? mode === 'merged' ? 'merged_confirmation_recorded' : 'explicit_start_confirmation_recorded' : mode === 'merged' ? 'merged_confirmation_required' : 'explicit_start_confirmation_required', 'human', task ? testPlanHash(task) : undefined, approvalRequired && requirementsConfirmed), gate('host_capabilities_ready', capabilityReady, capabilityReady ? 'host_capabilities_ready' : 'host_capability_missing', 'host', undefined, executionAuthorized), gate('task_assets_finalized', quickFinalizationCurrent, quickFinalizationCurrent ? 'task_assets_finalized' : 'task_finalization_pending', 'runtime', task?.finalization?.artifactHash, quickFinalizationRequired)];
  ...
  return { apiVersion: 'qa-agent/v3', kind: 'WorkflowState', request, moduleId, taskId, taskDirectory: taskReady ? `.qa-agent/modules/${moduleId}/tasks/${taskId}` : undefined, taskDirectoryAbsolute: taskReady ? taskDirectory(root, moduleId, taskId) : undefined, taskAssetsReady: taskReady, workflowStatus: status, taskState, workflowPhase: phase, confirmationMode: mode, reasonCode, gates, uiExecutionAllowed, mustStop, manualReportAllowed: false, runId: run?.status === 'running' ? run.id : undefined, plan: task ? taskPlan(task) : undefined, todoList: todos(moduleReady, taskReady, planReady, platformDeclared, questionsResolved, requirementsConfirmed, approvalRequired, executionAuthorized, capabilityReady, mode, run, quickFinalizationRequired, quickFinalizationCurrent), allowedActions, forbiddenActions, nextAllowedAction: nextActions[0]?.description ?? 'No action is currently allowed.', nextActions, breadcrumb: stateBreadcrumb, resumeToken: taskReady ? resumeToken(moduleId, taskId, run?.status === 'running' ? run.id : undefined, lastEvent?.seq, run?.status === 'running' ? progress : undefined) : undefined, contextHash };
}
