...
export function writeReport(root: string, task: TestTask, run: TestRun): string {
  ...
  const defectCandidates = [
    ...
    ...scenarioFailures.filter(item => !visualFailures.some(finding => finding.scenarioId === item.scenarioId)).map(item => ({ id: `execution-${item.scenarioId}`, type: 'execution-or-business-failure', scenario: item.scenarioId, expected: 'Declared scenario expectations', actual: item.detail ?? 'Scenario failed without additional detail.', screenshot: undefined as string | undefined })),
  ];
  ...
  const lines = [
    ...
    '> This report was generated by QA-Agent Runtime from the persisted Run. Manually written summaries are not authoritative.', '',
    '## Runtime 资产', '',
    `- Run ID: ${run.id}`,
    ...
    `- Screenshot count: ${run.screenshots?.length ?? 0}`,
    `- Evidence count: ${run.evidence?.length ?? 0}`, '',
    '## 测试上下文', '',
    ...
    `- Execution mode: ${run.mode ?? (run.replayStatus === 'not_replay' ? 'explore' : 'replay')}` , `- Git Branch: ${run.git.branch ?? 'unknown'}`, `- Git Commit: ${run.git.commit ?? 'unknown'}`, `- Safe Mode: ${run.safeMode}`, `- Test plan approval: ${task.metadata.approval ? `${task.metadata.approval.confirmedBy} at ${task.metadata.approval.confirmedAt} via ${task.metadata.approval.confirmationSource}` : 'missing'}`, `- Plan hash: ${task.metadata.approval?.planHash ?? 'unapproved'}`, `- Run plan hash: ${run.planHash ?? 'legacy-unknown'}`, `- Replay: ${run.replayStatus}${run.operationPlanId ? ` (${run.operationPlanId} v${run.operationVersion ?? '?'})` : ''}`, `- Replay stage: ${run.replayStage}`, `- MCP snapshot: ${run.context.mcpSnapshot.map(item => `${item.id}:${item.status}/${item.permissionStatus}`).join(', ') || 'none'}`, `- Permission snapshot: ${run.context.permissionSnapshot.status}`, '',
