...
export function writeReport(root: string, task: TestTask, run: TestRun): string {
  ...
  const lines = [
    ...
    `- Git Branch: ${run.git.branch ?? 'unknown'}`, `- Git Commit: ${run.git.commit ?? 'unknown'}`, `- Safe Mode: ${run.safeMode}`, `- Test plan approval: ${task.metadata.approval ? `${task.metadata.approval.confirmedBy} at ${task.metadata.approval.confirmedAt}` : 'missing'}`, `- Plan hash: ${task.metadata.approval?.planHash ?? 'unapproved'}`, `- Replay: ${run.replayStatus}${run.operationPlanId ? ` (${run.operationPlanId} v${run.operationVersion ?? '?'})` : ''}`, '',
    ...
    ...(run.operationCandidates?.length ? ['## Operation JSON 候选', '', ...run.operationCandidates.map(item => `- ${item}`), ''] : []),
