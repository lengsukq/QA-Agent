...
import { copySkill, copySubSkills, detected, managedTemplate, requireProjectPath, sharedGuidance, writeManagedText } from './shared.ts';
...
export const opencodeConfigurator: HostConfigurator = {
  configure(options) {
    ...
    copySkill(skill, force); const subskills = copySubSkills(join(skill, '..'), force); writeManagedText(command, sharedGuidance, force);
