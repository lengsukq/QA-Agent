...
import { copySkill, detected, managedTemplate, requireProjectPath, renderCopilotAgent, writeManagedText } from './shared.ts';
...
export const copilotConfigurator: HostConfigurator = {
  configure(options) {
    ...
    copySkill(skill, force); writeManagedText(agent, renderCopilotAgent(hostConfig('copilot')), force); writeManagedText(prompt, renderCopilotAgent(hostConfig('copilot')), force);
    return { host: 'copilot', paths: [skill, agent, prompt], message: 'Installed GitHub Copilot QA skill, custom agent, and prompt.' };
  },
