...
import { copySkill, copySubSkills, detected, managedTemplate, requireProjectPath, renderGeminiCommand, writeManagedText } from './shared.ts';
...
export const geminiConfigurator: HostConfigurator = {
  configure(options) {
    ...
    writeManagedText(command, renderGeminiCommand(hostConfig('gemini')), force); copySkill(shared, force); const subskills = copySubSkills(join(shared, '..'), force);
