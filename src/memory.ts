...
import { modulePath, qaPath } from './project.ts';
...
function memoryPath(root: string, moduleId: string | undefined, id: string): string {
  assertSafeId(id, 'memory id');
  return moduleId ? join(modulePath(root, moduleId), 'memory', `${id}.json`) : qaPath(root, 'shared-memory', 'entries', `${id}.json`);
}
...
export function createMemoryCandidate(root: string, input: Omit<ProjectMemory, '$schema' | 'status' | 'version' | 'createdAt' | 'updatedAt'>): { memory: ProjectMemory; conflicts: string[] } {
  return withFileLock(qaPath(root, '.locks', 'memories.lock'), () => {
    ...
    const path = memoryPath(root, input.moduleId, input.id);
    ...
    const timestamp = now();
    const memory: ProjectMemory = { $schema: '../../../schemas/memory.schema.json', ...input, content: redact(input.content), status: 'candidate', version: 1, createdAt: timestamp, updatedAt: timestamp };
    writeJsonAtomic(path, memory);
    return { memory, conflicts };
  });
}

export function reviewMemory(root: string, moduleId: string, id: string, decision: 'approve' | 'reject', level: ProjectMemory['knowledgeLevel'] = 'confirmed'): ProjectMemory {
  return withFileLock(qaPath(root, '.locks', 'memories.lock'), () => {
    const path = memoryPath(root, moduleId, id);
    ...
    for (const old of conflicts) {
      old.status = 'superseded'; old.updatedAt = now(); writeJsonAtomic(memoryPath(root, moduleId, old.id), old);
    }
...
export function curateFailedRun(root: string, task: TestTask, run: TestRun): string | undefined {
  ...
  const result = createMemoryCandidate(root, {
    id, moduleId: task.metadata.moduleId, type: 'known_issue', title: `Failed QA run: ${task.metadata.name}`, content,
...
export function curateObservedBusinessRules(root: string, task: TestTask, run: TestRun): string | undefined {
  ...
  const result = createMemoryCandidate(root, {
    id, moduleId: task.metadata.moduleId, type: 'business_rule', title: `Observed business outcome: ${task.metadata.name}`, content,
