...
import { taskDirectory, taskModuleSnapshotPath, taskPlanPath, taskPrdPath, taskRequirementsPath, taskSourceRunPath, taskSourceRunReportPath } from './project.ts';
...
export function inspectTaskArchive(root: string, task: TestTask): ArchiveInspection {
  ...
  check(checks, 'approval', 'Current execution approval', approved, approved ? [`planHash=${testPlanHash(task)}`, `confirmedBy=${task.metadata.approval?.confirmedBy}`] : ['Every Task must have the exact human start confirmation and a matching approved TestPlan']);
  ...
  const sourceRun = existsSync(sourceRunPath) ? (() => { try { return readJson<TestRun>(sourceRunPath); } catch { return undefined; } })() : undefined;
  const sourceResolved = Boolean(sourceRun?.completedAt && ['passed', 'adapted'].includes(sourceRun.status));
  check(checks, 'source-run', 'Source Run is completed and resolved', sourceResolved, sourceResolved ? [`${sourceRun!.id}: ${sourceRun!.status}`] : [sourceRun ? `${sourceRun.id}: ${sourceRun.status}; resolve and rerun before archive` : 'No Source Run exists']);
  const reportPath = taskSourceRunReportPath(root, task.metadata.moduleId, task.metadata.id);
  const evidenceOk = Boolean(sourceRun && sourceResolved && sourceRun.reportGeneratedBy === 'qa-agent-runtime' && existsSync(reportPath) && hasRuntimeReportMarker(readFileSync(reportPath, 'utf8'), sourceRun.id) && /!\[[^\]]*\]\([^\)]+\)/.test(readFileSync(reportPath, 'utf8')) && sourceRun.screenshots.length > 0 && sourceRun.screenshots.every(item => existsSync(join(directory, 'source-run', item.path))));
  check(checks, 'source-run-evidence', 'Source Run has report and screenshots', evidenceOk, evidenceOk ? [`${sourceRun!.id} has Runtime-owned evidence`] : ['The Source Run must be passed/adapted and include run.json, Runtime-owned report.md, existing screenshots, and Markdown image evidence']);
