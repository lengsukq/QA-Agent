...
import { approvalIsCurrent, confirmationMode, invalidateApproval, MERGED_CONFIRMATION_DISPLAY, PLAN_REVIEW_CONFIRMATION_DISPLAY, planReviewIsCurrent, START_CONFIRMATION_DISPLAY, testPlanHash } from './approval.ts';
...
export function applyPlanDraft(root: string, draft: PlanDraft): PlanDraftApplyResult {
  ...
  return { changed: true, moduleId: draft.moduleId, taskId: draft.taskId, planHash, previousPlanHash, requirementsConfirmationRequired: !updatedTask.requirements?.platformDeclaration || !planReviewIsCurrent(updatedTask), requiredRequirementsConfirmation: confirmationMode(updatedTask) === 'merged' ? MERGED_CONFIRMATION_DISPLAY : PLAN_REVIEW_CONFIRMATION_DISPLAY, platformDeclarationRequired: !updatedTask.requirements?.platformDeclaration, requiredPlatformDeclaration: PLATFORM_DECLARATION_PROMPT_ZH, platformDeclaration: updatedTask.requirements?.platformDeclaration, unresolvedQuestions: updatedTask.requirements?.userQuestions ?? [], approvalRequired: !approvalIsCurrent(updatedTask), confirmationMode: confirmationMode(updatedTask), requiredConfirmation: confirmationMode(updatedTask) === 'merged' ? MERGED_CONFIRMATION_DISPLAY : START_CONFIRMATION_DISPLAY, prdPath: taskPrdPath(root, draft.moduleId, draft.taskId), scenarioIds: updatedTask.scenarios.map(scenario => scenario.id), task: updatedTask };
}
