...
export function approveGuidedAction(root: string, runId: string, input: { scenarioId?: string; plannedStepId?: string; action?: string; expected?: string; confirmedBy: string; confirmationSource?: 'current-chat-explicit-approval' | 'external-review-record'; statement: string }): TestRun {
  ...
  if (task.metadata.mode !== 'guided') throw new Error(`Run ${runId} is not a user-led QA Run.`);
  ...
  if (run.guidedPending?.type === 'result_verdict') throw new Error(`User-led Run ${run.id} is waiting for the QA verdict on step ${run.guidedPending.stepId}.`);
  if (run.guidedPending?.type === 'execute_action') throw new Error(`User-led Run ${run.id} already has an approved action waiting to execute.`);
  if (run.completedAt) throw new Error(`User-led Run ${run.id} is complete.`);
  assertHumanApprover(input.confirmedBy);
  if (!input.statement.trim()) throw new Error('User-led action approval requires the QA reply that authorized this action.');
  const confirmationSource = input.confirmationSource ?? 'current-chat-explicit-approval';
  if (!['current-chat-explicit-approval', 'external-review-record'].includes(confirmationSource)) throw new Error('User-led action confirmationSource must be current-chat-explicit-approval or external-review-record.');
  const scenarioId = input.scenarioId ?? run.scenarioId ?? (task.scenarios.length === 1 ? task.scenarios[0]?.id : undefined);
  if (!scenarioId) throw new Error('User-led action approval must specify a Scenario when the Task contains multiple Scenarios.');
  ...
  if (!action || !expected) throw new Error('User-led action approval requires either --planned-step or both --action and --expected.');
  ...
  if (hasSecrets({ action, expected, statement: input.statement })) throw new Error('User-led action approval contains a potential secret.');
  const approval = {
    id: `guided-approval-${run.steps.filter(step => step.humanApproval).length + 1}`,
    confirmedBy: input.confirmedBy,
    confirmationSource,
    statement: input.statement.trim(),
    confirmedAt: now(),
  };
  run.guidedPending = { type: 'execute_action', scenarioId, plannedStepId: planned?.id, action, expected, approval };
  run.evidence.push({ type: 'qa-action-approval', summary: `${input.confirmedBy} approved user-led action: ${action}` });
  checkpointRun(root, run);
  return run;
}
...
export function recordGuidedVerdict(root: string, runId: string, input: { stepId: string; status: 'passed' | 'failed' | 'blocked' | 'paused' | 'inconclusive' | 'adapted'; confirmedBy: string; confirmationSource?: 'current-chat-explicit-approval' | 'external-review-record'; statement: string; note?: string }): TestRun {
  ...
  const task = readTask(root, run.moduleId, run.taskId);
  if (task.metadata.mode !== 'guided') throw new Error(`Run ${runId} is not a user-led QA Run.`);
  if (run.guidedPending?.type !== 'result_verdict' || run.guidedPending.stepId !== input.stepId) throw new Error(`User-led Run ${run.id} is not waiting for a verdict on step ${input.stepId}.`);
  assertHumanApprover(input.confirmedBy);
  if (!input.statement.trim()) throw new Error('User-led result verdict requires the QA reply that judged the observed result.');
  const confirmationSource = input.confirmationSource ?? 'current-chat-explicit-approval';
  if (!['current-chat-explicit-approval', 'external-review-record'].includes(confirmationSource)) throw new Error('User-led verdict confirmationSource must be current-chat-explicit-approval or external-review-record.');
  const step = run.steps.find(item => item.id === input.stepId);
  if (!step || step.source !== 'ui' || !step.humanApproval) throw new Error(`Step ${input.stepId} is not a user-led UI step.`);
  step.status = input.status;
  step.humanVerdict = { status: input.status, confirmedBy: input.confirmedBy, confirmationSource, statement: input.statement.trim(), note: input.note?.trim(), confirmedAt: now() };
  delete run.guidedPending;
...
export function recordAgentStep(root: string, runId: string, input: { action: string; plannedStepId?: string; uiAction?: UiAction; safetyAction?: string; detail: string; status?: RunStatus; screenshotPath?: string; visualInspection?: VisualInspectionStatus; source?: TestRun['steps'][number]['source']; executionMode?: StepExecutionMode; scenarioId?: string; locator?: Locator; actualLocator?: Locator; inputRefs?: Record<string, string>; expectedState?: string; actualState?: string; adaptation?: string }): TestRun {
  ...
  const guidedApproval = task.metadata.mode === 'guided' && source === 'ui' && run.guidedPending?.type === 'execute_action' ? run.guidedPending : undefined;
  const scenarioId = input.scenarioId ?? guidedApproval?.scenarioId ?? run.scenarioId ?? (task.scenarios.length === 1 ? task.scenarios[0]?.id : undefined);
  ...
  if (task.metadata.mode === 'guided' && source === 'ui') {
    if (!guidedApproval) throw new Error('User-led QA requires a QA-approved action before every UI operation. Use qa-agent run guide-approve first.');
    if (guidedApproval.scenarioId !== scenarioId) throw new Error(`The approved user-led action belongs to Scenario ${guidedApproval.scenarioId}, not ${scenarioId}.`);
  ...
  run.steps.push({
    id: stepId,
    plannedStepId: guidedApproval?.plannedStepId ?? input.plannedStepId,
    action: input.action,
...
export function completeAgentGuidedRun(root: string, task: TestTask, runId: string): TestRun {
  ...
  if (task.metadata.mode === 'guided') {
    ...
    if (run.guidedPending?.type === 'result_verdict') throw new Error(`User-led Run ${run.id} is waiting for the QA verdict on step ${run.guidedPending.stepId}.`);
    const missingVerdicts = run.steps.filter(step => step.source === 'ui' && (!step.humanApproval || !step.humanVerdict)).map(step => step.id);
    if (missingVerdicts.length) throw new Error(`User-led Run ${run.id} cannot complete until QA approves and confirms every UI result: ${missingVerdicts.join(', ')}.`);
    const missingScenarioExecution = task.scenarios
      .filter(scenario => !run.scenarioId || scenario.id === run.scenarioId)
      .filter(scenario => !run.steps.some(step => step.source === 'ui' && step.scenarioId === scenario.id && step.humanApproval && step.humanVerdict))
      .map(scenario => scenario.id);
    if (missingScenarioExecution.length) throw new Error(`User-led Run ${run.id} requires at least one confirmed UI step for every selected Scenario: ${missingScenarioExecution.join(', ')}.`);
  }
