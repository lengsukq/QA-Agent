...
当前版本：**v0.3.95**（[变更说明](skill/qa-agent/references/changelog.md)）

### v0.3.95 更新亮点

- **计划后声明平台** — 生成详细测试计划后，必须由 QA 明确选择 Web 或 iOS Simulator；平台声明写入 PlanDraft 后才能复审和执行。
- **平台声明可追踪** — 平台切换必须重新应用对应 PlanDraft，并可通过工作流状态看到阻塞原因和下一步操作。
...
- **全新初始化** — 0.3.95 不迁移旧 Runtime 资产；请重新初始化项目，Runner 由全局/npm 包解析。
...
5. QA 声明平台：Web 或 iOS Simulator
6. Agent 将平台写入 PlanDraft 并重新应用
7. QA 回复：确认测试方案
8. QA 回复：确认开始测试
9. Agent 执行测试并生成报告
```
