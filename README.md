...
v0.3.6 强化了测试产物可访问性、截图报告合同和回归脚本引导，并保留双重 PRD 审批和 QA 主导的 Guided 模式：
...
- PRD、测试报告和回归报告会通过可点击 Markdown 链接主动展示给用户；
- 正式测试报告和回归报告必须在 Markdown 内直接嵌入截图，单纯列出截图路径无效；
- 测试完成且满足条件时，Agent 必须主动询问是否生成 Python 回归脚本草稿；
- 回归脚本草稿生成与正式发布仍分别需要单独批准；
- `qa-agent-regression-test` 只运行已经发布的脚本；
- 正式 Python 回归脚本必须为每个来源 UI 步骤生成真实关键节点截图；
- 缺失、为空、路径错误或未覆盖来源步骤的截图会使回归结果变为 `invalid_result`；
- Agent 必须结合截图、预期状态与实际状态检查后，才能展示正式回归报告；
...
v0.3.6 继续采用精简资产模型，不生成重复的 `summary.md`、Quick 观察场景 JSON、Source Run 历史索引或 Session Journal。
