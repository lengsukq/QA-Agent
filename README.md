...
5. AI 主导模式评估完整流程是否具备生成 Python 回归脚本的条件；
6. 用户主导模式为每个已选 Scenario 自动生成一个独立脚本和 manifest。

Quick Task 会把结果和关键截图写回同一个 `prd.md` 并进入 `completed`。用户主导 Task 会把每个 Scenario 的脚本保存在：

```text
source-run/scenario-regressions/<scenario-id>/script.py
...
AI 主导模式只有在 Source Run 完成且证据充分时，才会询问是否生成完整流程脚本。用户主导模式不再询问同一个问题，因为 Runtime 已经按 Scenario 自动生成草稿。

无论来源是哪一种模式，正式发布都必须经过单独审核和批准。发布后的脚本先是 `approved_unverified`，至少完成一次合法执行合同后才成为 `validated`。后续 Task、Module 和 Release 回归由 `qa-agent-regression-test` 或 Runtime 回归命令执行。
