...
After every successful exploratory `qa-agent test` Run, actively call `qa-agent operation generate --module <module> --task <task> --run <run-id> [--scenario <scenario>]`. This command only writes the OperationPlan candidate; it does not approve, execute, or archive it. Show the candidate IDs, Scenario, source Run, plan hash, and readiness issues, then ask the user whether to approve them. After explicit approval, call `qa-agent task operation review <task> --module <module> --operation <operation-id> --approve`, then call `qa-agent test` again. An OperationPlan remains `validationStatus: unverified` until this real replay/adapted Run passes; only then is it valid and eligible for archive.
...
| `qa-agent operation generate --module MODULE --task TASK --run RUN_ID [--scenario SCENARIO]` | Only write a quick-regression OperationPlan candidate from a completed successful exploratory Run; approval and execution remain separate. | No |
...
6. After `run complete`, call `qa-agent operation generate` for the successful exploratory Run. A passed business result with no candidate is valid QA evidence but is not fast-replay ready. Report the exact missing replay fields instead of producing a separate manual PASS report.
7. Review and approve a candidate OperationPlan before adding it to a RegressionSuite. The plan is only valid after a successful replay/adapted Run sets `validationStatus: passed`.
