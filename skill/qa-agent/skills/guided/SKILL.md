...
7. Set `PlanDraft.executionIntent` to `state-changing` for Guided Tasks. Ask whether the PRD matches the requested behavior. Only the exact reply `确认测试方案` or `confirm test plan` may be persisted through `qa-agent plan review`.
8. Separately wait for the exact reply `确认开始测试` or `confirm start testing` (matching the user's language), persist it through `qa-agent review`, and only then call `qa-agent test`.
