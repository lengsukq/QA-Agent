...
计划生成后，Agent 读取源码、配置、入口、已安装目标和可用能力，自动判断唯一平台：Web 或 iOS Simulator，并把 `web`/`ios` 写入 `PlanDraft.platformDeclaration.platform`，将 `declaredBy` 设为 `qa-agent`，让 `scope.platforms` 保持一致后重新应用计划。只有无法唯一判断时才询问 QA。

计划可以直接调整，既有的方案确认和开始授权会保留；但与旧 Plan Hash 绑定的正式回归步骤会变为 `stale`，需要从新的成功 Run 导出步骤。平台切换时必须重新应用 PlanDraft，并重新走 Doctor、方案确认和开始授权。
...
当 PRD 已符合需求时，Agent 还必须在 PlanDraft 中明确设置：

```json
{ "executionIntent": "read-only" }
```

Runtime 根据执行意图、风险、测试数据和 Release Gate 计算 `confirmationMode`。普通只读流程满足条件时，QA 回复一次（根据用户语言选择）：

```text
确认测试并开始执行
confirm test and start execution
```

该确认同时记录方案确认和开始授权。

state-changing、high/critical、Guided、Release Gate 或涉及测试数据的流程，仍然需要：
...
严格模式下，“确认测试方案”只表示 PRD 正确，不代表允许操作设备。准备执行时，QA 还必须单独回复（根据用户语言选择）：
...
两个门禁都通过后，`qa-agent test` 只允许 Web 或 iOS Simulator，并检查：

- 当前 PRD、Plan Hash 和两次确认是否仍一致；
- 内置 Runner、Python、Playwright，或 iOS 的 `xcrun simctl`、`idb` 和已启动模拟器；
- 当前平台所需的本机权限；
- 环境、角色、测试数据和安全策略。
...
Agent 不得跳过声明的业务断言，也不得伪造截图、操作或结果。Agent 只能调用 `qa-agent act`；如果平台不匹配，先运行 `qa-agent doctor --platforms <web|ios>` 并重新应用 PlanDraft，不能调用 MCP 或其他 UI 工具绕过。
...
两者共享相同的计划和能力检查；确认次数由 Runtime 计算的 `confirmationMode` 决定。满足低风险只读条件时使用一次合并确认，否则使用严格双确认：
