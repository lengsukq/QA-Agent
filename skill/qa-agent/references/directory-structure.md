...
| `mcp.json` | 兼容性保留的宿主数据，不是 Web/iOS UI 执行依赖；Doctor 和 Run 使用本机内置 Runner 诊断。 |
...
| `skills/built-in/` | Runtime 内部能力合同，例如执行、证据、报告、Memory 和 Regression。它们不是用户对话中的三个宿主 Skill。 |
...
| `task.json` | Task 的轻量主清单：名称、状态、优先级、模式、审批策略，以及对 Scenario、Source Run、报告和回归脚本的引用。 |
...
| 文件或目录 | 用途 |
| --- | --- |
| `source-run/run.json` | 首次真实业务测试的结构化事实，包含环境、Git 状态、真实步骤、定位器、截图引用、断言、Cleanup、恢复过程、结论和回归步骤生成资格。 |
...
正式回归脚本发布前，再次进行首次测试会替换旧的未冻结 Source Run，并在 `events.jsonl` 记录 `source_run_restarted`。正式脚本发布后，Source Run 被冻结，后续执行只能进入 `regression-runs/`。Task 不维护多个 `runs/<run-id>/` 探索历史。
...
| `.runtime/drafts/<session>/<script-id>/<script-id>.steps.json` | 尚未发布的回归步骤草稿。它不会参加 Task、Module 或 Release 回归，发布后草稿目录会删除。 |
| `regression/<script-id>.steps.json` | 用户审阅并批准后的正式回归步骤脚本，由统一执行器一键重放，固定后续执行顺序。 |
...
| `stdout.log` / `stderr.log` | 回归执行进程的标准输出和错误输出，用于排查步骤脚本、执行器或环境问题。 |
| `screenshots/` / `evidence/` | 正式脚本本次执行产生的截图和其他证据。每个来源 UI 步骤必须有新的关键节点截图，并在 `result.json` 中引用。 |

TestPlan 改变时，与旧 Plan Hash 绑定的正式脚本会标记为 `stale`，不会继续进入回归选择；既有方案审批不会因为普通计划或平台调整自动失效。重新生成时需要新的 Source Run、脚本草稿审阅和发布批准；只有新增未决业务问题时才需要重新确认计划。
